...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let session = Session::create(SessionOptions {
        client_name: Some("ANDROID_TESTSUITE".to_string()),
        client_version: Some("1.9".to_string()),
        device_category: Some("MOBILE".to_string()),
        user_agent: Some("google-youtube-dl/1.0".to_string()),
    ...
    println!("ANDROID_TESTSUITE Status: {}", resp.status());
    let pr: PlayerResponse = resp.json().await?;
    println!("ANDROID_TESTSUITE Playability: {} (reason: {:?})", pr.playability_status.status, pr.playability_status.reason);
