...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let session = Session::create(SessionOptions {
        client_name: Some("WEB_EMBEDDED_PLAYER".to_string()),
        client_version: Some("1.20260206.01.00".to_string()),
    ...
    println!("WEB_EMBEDDED_PLAYER Status: {}", resp.status());
    let pr: PlayerResponse = resp.json().await?;
    println!("WEB_EMBEDDED_PLAYER Playability: {} (reason: {:?})", pr.playability_status.status, pr.playability_status.reason);
