...
impl Actions {
    ...
    pub async fn subscribe(session: &Session, channel_ids: &[&str]) -> Result<ActionResult> {
        ...
        let resp = session.post_innertube("/subscription/subscribe", payload).await?;
        let raw: Value = resp.json().await.map_err(InnertubeError::Network)?;
    ...
    pub async fn unsubscribe(session: &Session, channel_ids: &[&str]) -> Result<ActionResult> {
        ...
        let resp = session.post_innertube("/subscription/unsubscribe", payload).await?;
        let raw: Value = resp.json().await.map_err(InnertubeError::Network)?;
    ...
    pub async fn create_playlist(
        ...
        let playlist_id = raw.get("playlistId").and_then(Value::as_str).map(|s| s.to_string());
    ...
    pub async fn add_to_playlist(
        ...
        let actions: Vec<Value> = video_ids.iter().map(|vid| json!({
        ...
        let resp = session.post_innertube("/browse/edit_playlist", payload).await?;
        let raw: Value = resp.json().await.map_err(InnertubeError::Network)?;
    ...
    pub async fn remove_from_playlist(
        ...
        let actions: Vec<Value> = set_video_ids.iter().map(|set_id| json!({
    ...
    pub async fn create_comment(
        ...
        let payload = json!({
            ...
            "createCommentParams": video_id,
        });

        let resp = session.post_innertube("/comment/create_comment", payload).await?;
        let raw: Value = resp.json().await.map_err(InnertubeError::Network)?;
